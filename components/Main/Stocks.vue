            ...
            <div class="slader__item">
                <div class="relative">
                    ...
                    sm:w-11/12 sm:top-1/4">
                        ...
                        :class="elem.title_col">{{  elem.title }}</h3>
                        <p class="text-lg text-gray-300 mb-6
                            lg:text-base lg:mb-3 md:text-2xl sm:text-lg">{{ elem.text }}</p>
                        <div class="flex md:hidden">
                            <ButtomStandart
                            class="bg-blue-500 text-white mr-4 px-16 py-2.5 lg:px-10">
                                Подробнее
                            </ButtomStandart>
                            <ButtomArround  @click="prevItem" class="-scale-100 group big xl:py-3"/>
                            <ButtomArround @click="nextItem" class="ml-2 group big xl:py-3"/>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <template v-if="isMobile">
                    <div class="py-4">
                        <div class="flex justify-end">
                            <ButtomStandart
                            class="bg-blue-500 text-white mr-4 text-center py-3 text-xl grow
                            sm:text-base sm:py-2">
                            Подробнее
                        </ButtomStandart>
                        <ButtomArround  @click="prevItem" class="-scale-100 group big px-6 sm:px-5"/>
                        <ButtomArround @click="nextItem" class="ml-2 group big px-6 sm:px-5"/>
                    </div>
                </div>
            </template>
            <div></div>
