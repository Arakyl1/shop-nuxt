<template>
    <div class="relative">
        ...
        @mouseenter="showModalCategor" @mouseleave="hudeModalCategor">
            <div >
            <UIStandart class=" text-white flex items-center h-14 relative after:z-[-1] lg:h-12"
            :class="{ 'after:content-none': stage}">
                <UIIconMenu class="group is-icon-white mr-3"/>
                <NuxtLink to="/catalog">
                    Каталог
                </NuxtLink>
            </UIStandart>
        ...
        <div class="pl-36 h-14 w-full bg-gray-100 relative lg:h-12">
            <div>
                ...
                <div
                  ...
                  @click="listOffset">
                    <UIIconArround :class="{ 'rotate-180': !active }" class="header__aditional-icon transition-all"/>
                </div>
            </div>
        </div>
    </div>
