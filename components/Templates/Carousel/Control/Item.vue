<template>
        <ul ref="ul" :class="$style.body" @click.stop="onClick">
            <li v-for="item in data" :key="item" v-bind="{ [dataKey('index')]: (item - 1) }" :class="[itemClass, $style.item]">
                <slot v-bind="{ item, indexActiveButton, ul }">
                    <UITGeometryCircle data-control-element-circle :class="[indexActiveButton === (item - 1) ? 'bg-blue-500' : 'bg-blue-100']" class="h-4 w-4"/>
            </slot>
        </li>
    </ul>
</template>
<script setup lang="ts">
import { UITGeometryCircle } from "#components";
import type { RecordOption } from '~~/type/intex'
...
function onClick({ target }: MouseEvent) {
    if (ul.value) {
        ...
        const _targetElem = target as HTMLElement
        const _liElem = _targetElem.closest(`[${dataKey('index')}]`)
        if (!_liElem) return

        const _elem: ModifiedHTMLElement = _liElem as ModifiedHTMLElement
        const dataIndex: number = +_elem.dataset.index
        const conWid: number = +ul.value.clientWidth.toFixed(0),
