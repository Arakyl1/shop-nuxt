<template>
    <section class="aspect-[228/101] w-full">
        <EntitiesOtherSladerBase :data="data.body">
                    ...
                    <div class="relative">
                        ...
                        <div class="absolute w-[40%] h-min top-1/2 left-[10%]
                            ...
                            <div class="flex md:hidden">
                                <UIStandart class="bg-blue-500 text-white mr-4 px-16 py-2.5 lg:px-10">
                                    Подробнее
                                </UIStandart>
                                <SharedUIButtomArround @click="prevItem" class="-scale-100 group big xl:py-3" />
                                <SharedUIButtomArround @click="nextItem" class="ml-2 group big xl:py-3" />
                            </div>
                        </div>
                    </div>
                </template>
                <EntitiesItemBase v-else class="aspect-[3/2]"/>
            </template>
            <template #bottom="{ nextItem, prevItem }">
                <template v-if="isMobile">
                    <div class="py-4">
                        <div class="flex justify-end">
                            <UIStandart class="bg-blue-500 text-white mr-4 text-center py-3 text-xl grow
                            sm:text-base sm:py-2">
                                Подробнее
                            </UIStandart>
                            <SharedUIButtomArround @click="prevItem" class="-scale-100 group big px-6 sm:px-5" />
                            <SharedUIButtomArround @click="nextItem" class="ml-2 group big px-6 sm:px-5" />
                        </div>
                    </div>
                </template>
                <div></div>
