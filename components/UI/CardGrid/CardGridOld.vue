<template>
    <ul :class="rootClass"> 
        ...
        <template v-else>
            <li  v-for="section in list ? ($slots.default())[0].children : $slots.default()">
...
const props = defineProps({
    ...
    // если передается v-for список обязательно указать этот параметр
    // если в slot передать v-for список он отрендерится как один элемент
    // а если передать просто несколько компонентов, то проблем не возник
    // каждый переданый компонент будет как будет отдельным элементом
    list: {
        type: Boolean
    }
})
